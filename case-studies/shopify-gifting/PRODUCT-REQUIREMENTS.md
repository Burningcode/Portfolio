...
Brands had demand for creator gifting but relied on messages and spreadsheets to coordinate product fit, variants, delivery information, orders, fulfillment, and reporting. Creators could request products that were not available for gifting, creating avoidable back-and-forth for both sides. The proposed MVP connects the Creator Commerce platform to Shopify so brands can define eligible products and limits, creators can confirm one or more valid product selections, and Shopify can create and fulfill the resulting order without routine manual entry.
...
If brands can define Shopify-backed gifting rules and creators can complete valid product selections that automatically enter normal fulfillment, then participating brands will complete more gifting collaborations with less active handling time and use more allocated inventory, because routine product coordination and order entry will no longer scale linearly with volume.

### Behavior and business outcome

- **Behavior to create:** A creator confirms one selection set containing one or more eligible products and variants, which accepts the collaboration and produces one traceable fulfillment flow without routine brand intervention.
...
| **MVP 1.0** | Order completion | Unique collaborations producing one authoritative Shopify order divided by collaborations with a confirmed valid selection |
...
| **Fast Follow 1.2** | Application acceptance rate | Applicants selected for an offer divided by reviewed eligible applications; selection of valid products remains collaboration acceptance |
...
| --- | --- | --- | --- |
| **MVP 1.0: Shopify fulfillment** | An invited creator confirms one or more valid product selections and reaches the brand's normal Shopify fulfillment flow | Gifting authorization, eligible catalog, quantity or value limits, selection-as-acceptance, Shopify checkout, basic gift identifier, fulfillment synchronization, transaction ledger, product analytics, recovery, and production controls | No marketplace applications, custom reconciliation rules, post-fulfillment messaging, or embedded ordering experience |
...
4. **Select and accept:** The creator confirms one or more valid product and variant selections within the brand's configured quantity or value limits. Confirmation records collaboration acceptance atomically.
5. **Validate:** The platform revalidates connection, offer, catalog version, product eligibility, limits, variants, and inventory.
6. **Checkout and create:** Shopify checkout collects or confirms delivery information and creates the initial order with a basic gift identifier.
...
| Offer | Creator Commerce platform | `DRAFT -> OFFERED -> SELECTION_PENDING`; it may transition to `EXPIRED`, `WITHDRAWN`, or `CANCELLED` before acceptance |
| Collaboration | Creator Commerce platform | A confirmed valid selection atomically creates `ACCEPTED`; then `ACCEPTED -> CHECKOUT_PENDING -> ORDER_PENDING -> ORDERED -> FULFILLED`; recoverable failure enters `EXCEPTION` with the last valid state retained |
| Fulfillment transaction | Creator Commerce platform ledger for transaction state; Shopify for commerce outcome | `NOT_STARTED -> CHECKOUT_OPEN -> SUBMITTED -> ORDER_RECORDED -> FULFILLED`; ambiguous writes enter `RECONCILIATION_REQUIRED`; a retry cannot create a second accepted order reference |
...
- One confirmed valid selection set creates at most one collaboration-acceptance event.
- One completed checkout creates at most one authoritative Shopify order reference for the collaboration.
...
| M10-6 | The brand chooses which creator receives a private, time-bounded offer | Preserve program judgment | Only the intended creator can enter the offer's selection flow |
| M10-7 | An invited creator can confirm one selection set containing one or more allowed products and variants; confirmation records collaboration acceptance | Consolidate informed choice and acceptance | One confirmed set produces one acceptance event; retry is idempotent; browsing never accepts |
| M10-8 | The system revalidates authorization, offer state, catalog version, eligibility, limits, variants, and inventory before checkout | Prevent scalable errors | Invalid state blocks progress with an actionable recovery path |
| M10-9 | One completed Shopify checkout produces at most one initial order with a basic gift identifier | Enter normal fulfillment without duplicate cost | The platform stores one authoritative order reference before marking the transaction `ORDER_RECORDED` |
...
| M10-13 | Material connection, catalog, inventory, checkout, order, and synchronization failures enter a visible recovery workflow | Prevent silent loss at productized scale | Each failure records cause, last valid state, owner, next action, retry status, and resolution |
...
| F12-5 | A selected applicant enters the same product-selection flow as an invited creator, and confirming a valid selection records collaboration acceptance | Preserve one canonical acceptance model | Application source does not change the selection, acceptance, checkout, or fulfillment state transitions |
...
| F13-4 | Retry, timeout, ambiguous result, and order failure preserve the accepted collaboration and valid selection | Prevent partner failure from destroying customer work | Reconciliation occurs before another order reference is accepted; recovery resumes from the last valid state |
...
- Implement state changes as explicit domain events. One valid selection set creates at most one accepted collaboration, and one completed fulfillment instruction creates at most one authoritative order.
...
- Pass M10-1 through M10-13 and Q1 through Q8 for the enabled surface.
...
| --- | --- | --- | --- | --- |
| MVP 1.0 | Stale inventory, invalid variant, or duplicate order | Reselection, checkout failure, or duplicate reference | Revalidate, use idempotency, preserve state, and reconcile before retry | Engineering |
...
- Treat one confirmed set containing one or more valid product selections as collaboration acceptance.
